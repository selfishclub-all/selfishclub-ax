...
### 미결정 / 논의 필요
- **카카오 로그인 vs 카카오싱크**: 현재는 카카오 로그인(일반 OAuth)으로 구현되어 있다. 카카오싱크를 도입하면 동의항목 한 번에 수집, 카카오톡 채널 친구 추가 연동, 간편가입 등이 가능하지만 비즈앱 + 추가 기능 신청 심사가 필요하다. 전화번호 기반 기존 회원 매핑을 할 때 카카오싱크가 유리할 수 있으므로, 회원 매핑 로직 구현 전에 결정 필요.
- **Supabase 유료 전환 시점**: 현재 무료 플랜(1GB 저장 + 2GB 전송/월). 이미지 저장소도 Supabase Storage 사용 중. 트래픽/데이터 증가 시 Pro($25/월) 전환 필요. 전환 시점과 비용 구조 사전 검토 필요.
- **Webflow 상세 페이지 이미지**: 현재 OG 이미지(썸네일)를 가져오고 있는데 디자인이 별로. 본문 이미지는 CDN 핫링크 차단으로 접근 불가. CMS에 저장된 이미지만 사용 가능한 상황. 더 나은 이미지 소스를 찾거나, 이미지 없이 텍스트 기반으로 갈지 결정 필요.
