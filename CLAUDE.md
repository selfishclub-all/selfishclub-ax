...
- 카카오 로그인 vs 카카오싱크 — 현재는 일반 OAuth. 카카오싱크 도입 시 동의항목 일괄 수집, 채널 친구 추가 연동 가능. 전화번호 기반 기존 회원 매핑 전에 결정 필요
- Webflow 상세 페이지 이미지 소스 — OG 이미지(썸네일)는 디자인이 별로, 본문 이미지는 CDN 핫링크 차단. 더 나은 소스를 찾거나 텍스트 기반으로 갈지 결정 필요
