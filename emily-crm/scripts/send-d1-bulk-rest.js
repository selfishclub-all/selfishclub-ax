// AAA(iid_1002) D-1 발송 — 첫 발송 누락분 (rows[1000..]) 추가 발송
...
const TEMPLATE_ID = 'KA01TP250810153246575dLXbwnUflWd'
const IID = 'iid_1002'
...
async function main() {
  console.log('=== iid_1002 전체 페이지네이션 조회 ===')
  const allRaw = await fetchAll(`event?select=u_name,u_phone&iid=eq.${IID}`)
  console.log(`event raw: ${allRaw.length}`)

  // 첫 1000 raw rows = 이미 발송됨 (이전 send-d1-bulk.js 실행)
  const firstChunk = allRaw.slice(0, 1000)
  const restChunk = allRaw.slice(1000)

  const sentPhones = new Set()
  for (const r of firstChunk) {
    const p = (r.u_phone || '').trim()
    if (p && /^010\d{8}$/.test(p)) sentPhones.add(p)
  }
  console.log(`이미 발송 (첫 1000 raw → unique): ${sentPhones.size}`)

  // 나머지 raw rows에서 유효+미발송 phones만 추출
  const seen = new Set(sentPhones)
  const recipients = []
  for (const r of restChunk) {
    const p = (r.u_phone || '').trim()
    if (!p || !/^010\d{8}$/.test(p)) continue
    if (seen.has(p)) continue
    seen.add(p)
    recipients.push({ phone: p, name: r.u_name || '회원' })
  }
  console.log(`추가 발송 대상 (rest unique - sent): ${recipients.length}`)
  if (recipients.length === 0) { console.log('추가 발송 없음'); return }
  ...
  console.log(`발송 시작 — ${recipients.length}명 / ${batches.length} 배치`)
